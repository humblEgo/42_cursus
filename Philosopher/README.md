...
9. 사용면에서는 뮤텍스가 ㅅ마포어에 비해 좀 더 간단한 사용법을 갖는다.
...
#### 해결) 모니터 쓰레드를 생성하다가 종료되는 경우, 철학자 쓰레드를 생성하다가 종료되는 경우, 그리고 두 쓰레드를 생성하기 전에 종료되는 경우로 케이스를 나누어서 destory할 mutex 개수를 구하고, 이를 바탕으로 destroy하도록 만들었다.
